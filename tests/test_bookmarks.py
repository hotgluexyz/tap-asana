        ...
        for stream in first_sync_bookmarks.get("bookmarks").keys():
            if self.is_incremental(stream):
                ...
                new_state["bookmarks"][stream][
                    next(iter(replication_keys[stream]))
        ...
        for stream in expected_streams:
            ...
            with self.subTest(stream=stream):
                ...
                if self.is_incremental(stream):
                    ...
                    first_bookmark_value_parsed = parse(first_bookmark_value).strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    )
                    second_bookmark_value_parsed = parse(
                    ...
                    for record in second_sync_messages:
                        ...
                        replication_key_value_parsed = parse(
                            replication_key_value
                        ).strftime("%Y-%m-%dT%H:%M:%SZ")
                        self.assertLessEqual(
                            replication_key_value_parsed,
                            second_bookmark_value_parsed,
                        ...
                        self.assertGreaterEqual(
                            replication_key_value_parsed,
                            parse(self.second_start_date).strftime(
                                "%Y-%m-%dT%H:%M:%SZ"
                            ),
